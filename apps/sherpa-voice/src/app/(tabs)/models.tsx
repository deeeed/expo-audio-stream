...
const logger = baseLogger.extend('ModelsScreen');
...
export default function ModelsScreen() {
  ...
  const handleModelBrowse = async (modelPath: string) => {
    try {
      logger.info(`action: browse model at ${modelPath}`);
      ...
      if (!info.exists) {
        ...
        if (Platform.OS === 'ios') {
          ...
          logger.info(`Trying reconstructed path for model: ${modelId}`);

          const newInfo = await FileSystem.getInfoAsync(newPath);
          if (newInfo.exists) {
            logger.info('Reconstructed path exists, using it');
      ...
      logger.error(`Error browsing model: ${error instanceof Error ? error.message : String(error)}`);
  ...
  const browseWithValidPath = (modelPath: string, info: FileSystem.FileInfo) => {
    ...
    logger.info(`Browsing model at: ${browsePath} (isDirectory: ${info.isDirectory})`);
