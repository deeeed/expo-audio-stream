...
print_launch_hint() {
  if [ -n "$LAN_IP" ]; then
    ...
    echo "  adb shell am start -a android.intent.action.VIEW -d \"exp+sherpa-voice://expo-development-client/?url=http://${LAN_IP}:${PORT}\" net.siteed.sherpavoice/.MainActivity"
    echo ""
  fi
}
