#!/bin/bash
# preflight.sh — Full environment setup for sherpa-voice app.
...
#   WATCHER_PORT     Metro port (default: 7500)
...
# Look for identifier blocks with udid and reality=physical
...
" 2>/dev/null || true)
  ...
  if [ "$USE_IOS_PHYSICAL" = false ]; then
    SIM="${IOS_SIMULATOR:-sherpa-voice-1}"
...
# Determine Metro host based on device type
if [ "$PLATFORM" = "ios" ] && [ "${IOS_DEVICE_MODE:-simulator}" = "physical" ]; then
...
if [ "$PLATFORM" = "ios" ] && [ "${IOS_DEVICE_MODE:-simulator}" = "physical" ]; then
  # Launch on physical device via devicectl
