...
import { useSpeakerIdModelWithConfig, useSpeakerIdModels } from './useModelWithConfig';
...
export function useSpeakerId() {
  ...
  const handleInitSpeakerId = async () => {
    ...
    try {
      ...
      if (!result.success) throw new Error(result.error || 'Unknown error during speaker ID initialization');
  ...
  const handleProcessAudio = async (audioItem: SpeakerIdAudioFile) => {
    if (!initialized) { setError('Speaker ID is not initialized'); return; }
    ...
    try {
      ...
      if (!result.success) throw new Error(result.error || 'Unknown error during audio processing');
      ...
      if (speakerCount > 0) {
        ...
        const idResult = await SpeakerId.identifySpeaker(result.embedding, threshold);
        setIdentifyResult(idResult);
        setStatusMessage(idResult.identified ? `Speaker identified: ${idResult.speakerName}` : 'No matching speaker found');
      }
    } catch (err) {
  ...
  const handleRegisterSpeaker = async () => {
    if (!initialized || !embeddingResult) { setError('Speaker ID is not initialized or no embedding available'); return; }
    if (!newSpeakerName.trim()) { setError('Please enter a name for the speaker'); return; }
    ...
    try {
      ...
      if (!result.success) throw new Error(result.error || 'Unknown error during speaker registration');
  ...
  const handleRemoveSpeaker = async (name: string) => {
    ...
    try {
      ...
      if (!result.success) throw new Error(result.error || 'Unknown error during speaker removal');
  ...
  const handleSelectAudio = async (audioItem: SpeakerIdAudioFile) => {
    ...
    try {
      ...
      tempPlayer.remove();
      setAudioMetadata({ size: fileSize, duration: durationMs, isLoading: false });
    } catch (_) {
