...
import { useModels, useSpeakerIdModelWithConfig, useSpeakerIdModels } from './useModelWithConfig';
...
export function useDiarization() {
  ...
  const [selectedSegModelId, setSelectedSegModelId] = useState<string | null>(null);
  const [selectedEmbModelId, setSelectedEmbModelId] = useState<string | null>(null);
  ...
  const [loadedAudioFiles, setLoadedAudioFiles] = useState<DiarizationAudioFile[]>([]);
  const [selectedAudio, setSelectedAudio] = useState<DiarizationAudioFile | null>(null);

  const { downloadedModels: segModels } = useModels({ modelType: 'diarization-segmentation' });
  const { downloadedModels: embModels } = useSpeakerIdModels();
  const { getModelState } = useModelManagement();
  const { speakerIdConfig } = useSpeakerIdModelWithConfig({ modelId: selectedEmbModelId });
  ...
  const resolveModelDir = async (rawPath: string): Promise<string> => {
    ...
    try {
      ...
      const sherpaDir = dirContents.find(item => item.includes('sherpa-onnx') || item.includes('pyannote'));
      if (sherpaDir) {
  ...
  const handleInit = async () => {
    const segModelState = selectedSegModelId ? getModelState(selectedSegModelId) : undefined;
    const embModelState = selectedEmbModelId ? getModelState(selectedEmbModelId) : undefined;
    ...
    try {
      ...
      const cleanEmbPath = embModelState.localPath.replace(/^file:\/\//, '');
      ...
      if (result.success) {
        setInitialized(true);
        setStatusMessage(`Initialized (sample rate: ${result.sampleRate} Hz)`);
      } else {
  ...
  const handleProcessFile = async (audioFile: DiarizationAudioFile) => {
    if (!initialized) { setError('Initialize diarization first'); return; }
    ...
    try {
      ...
      if (result.success) {
        ...
        setStatusMessage(`Found ${result.numSpeakers} speaker(s) in ${result.durationMs}ms`);
      } else {
