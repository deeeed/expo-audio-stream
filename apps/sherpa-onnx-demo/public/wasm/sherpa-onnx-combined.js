  ...
  const loadModulesSequentially = async function() {
    ...
    try {
      ...
      if (global.onSherpaOnnxReady) {
        ...
        console.log("Sherpa-ONNX modules loaded, checking FS availability:", 
                    allLoaded, "FS available:", !!global.Module?.FS);
        global.onSherpaOnnxReady(allLoaded && !!global.Module?.FS);
      }
    } catch (error) {
      console.error("Error during module loading:", error);
      
      if (global.onSherpaOnnxReady) {
        global.onSherpaOnnxReady(false);
      }
    } finally {
