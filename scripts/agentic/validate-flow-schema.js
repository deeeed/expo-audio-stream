...
function validateActionShape(node, issues) {
  switch (node.action) {
    ...
    case 'screenshot':
      if (
        Object.prototype.hasOwnProperty.call(node, 'max_diff_pixels') &&
        Number.isNaN(Number(node.max_diff_pixels))
      ) {
        issues.push(`  [${node.id || '?'}] screenshot max_diff_pixels must be numeric`);
      }
      if (
        Object.prototype.hasOwnProperty.call(node, 'max_diff_ratio') &&
        Number.isNaN(Number(node.max_diff_ratio))
      ) {
        issues.push(`  [${node.id || '?'}] screenshot max_diff_ratio must be numeric`);
      }
      if (
        Object.prototype.hasOwnProperty.call(node, 'pixel_threshold') &&
        Number.isNaN(Number(node.pixel_threshold))
      ) {
        issues.push(`  [${node.id || '?'}] screenshot pixel_threshold must be numeric`);
      }
      break;
    case 'switch':
