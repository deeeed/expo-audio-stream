  ...
  SherpaOnnx.FileSystem = {
    ...
    safeCreateDirectory: function(dirPath, debug = false) {
      try {
        ...
        // Create each path component, skipping those that already exist
        let currentPath = '';
        ...
        for (const part of parts) {
          ...
          try {
            ...
            if (debug) console.log(`Created directory: ${currentPath}`);
          } catch (mkErr) {
            // Emscripten uses errno=20 for EEXIST (not POSIX 17)
            if (mkErr.errno !== 20 && mkErr.errno !== 17 && mkErr.code !== 'EEXIST') {
              throw mkErr;
            }
          }
        }
